...
class AddressBook  {
	...
	public void addContact() throws IOException {
		...
		case "I":  // If contact is internationally located - create an international contact
			...
			// set Address
			System.out.println("Enter contact's address (one field at a time - starting with street number, street name, city, state, zipcode): ");
			interPerson.setAddress(input.nextLine(),input.nextLine(),input.nextLine(),input.nextLine(),input.nextLine());
	...
	public ArrayList<Person> updateContact() {
		...
		if (searchResult == -1) {
			...
			System.out.println("Enter contact's address (one field at a time - starting with street number, street name, city, state, zipcode): ");
			contacts.get(searchResult).setAddress(input.nextLine(), input.nextLine(), input.nextLine(), input.nextLine(), input.nextLine()); // set address
			System.out.println();
